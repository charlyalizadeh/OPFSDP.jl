function _tr_power_balance_real_leq_nbus!(model, nbus, W::DefaultDict, Y, bus_id, d)
...
function _tr_power_balance_real_g_nbus!(model, nbus, W::DefaultDict, Y, bus_id, d)
    return @expression(model,
...
function _tr_power_balance_real!(model, network, W::DefaultDict, Y, bus_id, d)
    _nbus = nbus(network)
...
function _tr_power_balance_imag_leq_nbus!(model, nbus, W::DefaultDict, Y, bus_id, d)
    return @expression(model,
                       sum(W["$(j), $(d)"] * (-1/2) * imag(Y[bus_id, j])  +
                           W["$(nbus + j), $(d)"] * (-1/2) * real(Y[bus_id, j])
                           for j in union(1:bus_id-1, bus_id+1:nbus)) -
...
function _tr_power_balance_imag_g_nbus!(model, nbus, W::DefaultDict, Y, bus_id, d)
    return @expression(model,
                       sum(W["$(j), $(d)"] * 1/2 * real(Y[bus_id, j])  +
                           W["$(nbus + j), $(d)"] * (-1/2) * imag(Y[bus_id, j])
                           for j in union(1:bus_id-1, bus_id+1:nbus)) -
...
function _tr_power_balance_imag!(model, network, W::DefaultDict, Y, bus_id, d)
    _nbus = nbus(network)
    if !(d in [bus_id, _nbus + bus_id])
        if d <= _nbus
            return @expression(model,
...
#function _define_power_balance_constraints!(model, network, variables, W::DefaultDict, Y)
#    constraints = []
